...
import { Bar, Cell, BarChart as RCBarChart, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts';
...
export const BarChart: React.FC<ChartProps> = ({ className, data }) => {
  ...
  const [highlightedMonth, setHighlightedMonth] = useState(HIGHLIGHTED_MONTH_INDEX);

  return (
    <div className={cn('venus-hero-earnings-chart w-full h-32', className)}>
      <ResponsiveContainer>
        <RCBarChart
          margin={{
            top: 0,
        ...
        >
          <Bar
            ...
            onMouseEnter={(_, index) => setHighlightedMonth(index)}
            radius={[9999, 9999, 9999, 9999]}
          >
            {data.map((point, index) => (
              <Cell key={point.months} fill={index === highlightedMonth ? theme.colors.blue : 'var(--hero-bar-muted, #181D27)'} />
            ))}
          </Bar>
