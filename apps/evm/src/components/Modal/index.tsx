...
export const Modal: FC<ModalProps> = ({
  ...
  return createPortal(
    ...
    >
      <div
        className={cn(
          'venus-popup-backdrop fixed inset-0 z-9999 backdrop-blur-xs',
          backdropClassName,
        )}
      ...
      <div className={cn('pointer-events-none fixed inset-0 z-99999', rootClassName)}>
        <div
          className={cn(
            'venus-popup-panel pointer-events-auto absolute top-1/2 left-1/2 flex max-h-[calc(100%-2rem)] w-[calc(100%-2rem)] max-w-136 -translate-x-1/2 -translate-y-1/2 flex-col overflow-auto rounded-xl border border-blue bg-dark-blue outline-hidden',
        ...
        >
          {hasHeader && (
            <div
              className={cn(
                'venus-popup-header sticky top-0 z-10 flex items-center justify-center rounded-t-2xl px-6 pt-6',
                handleBackAction && 'venus-popup-header-back',
